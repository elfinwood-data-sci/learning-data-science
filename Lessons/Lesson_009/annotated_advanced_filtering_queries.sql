...
-- 1) Basic WHERE clause using the equal sign. Returns only those rows where the soil_restrict_layer_code column equals 'permafrost'
/*SELECT * FROM public.site
	WHERE soil_restrict_layer_code = 'permafrost'*/

-- 2) Basic WHERE clause using the greater than or equal to sign. Returns only those rows where the elevation_m column contains a value greater than or equal to 1000
/*SELECT
  study_value,
  sample_id,
  elevation_m
...
-- 3) WHERE clause using the AND operator. Returns only those rows where the elevation_m column is greater than or equal to 1000 and less than or equal to 1200
/*SELECT
  study_value,
  sample_id,
  elevation_m
...
-- 4) WHERE clause using the BETWEEN comparison predicate to express the range of values we are interested in filtering on. 
...
-- 5) WHERE clause using the IS NOT NULL predicate. The results will contain only those rows for which the site_field_note column has a value (i.e., is not null).
...
-- 6) WHERE clause using the OR operator. The following query will return only those rows where the physiography_value column equals a or u.
/*SELECT study_value, sample_id, physiography_value
FROM public.site
WHERE physiography_value = 'a' OR physiography_value = 'u'*/

-- 7) WHERE clause using the IN operator. The IN operator uses a set of one or more values as the evaluation criteria. 
...
-- 8) WHERE clause using the IN operator with the NOT operator, i.e., NOT IN, which is the converse of IN. 
...
-- 9) WHERE clause using the LIKE operator with the percent wildcard character following the literal characters. 
...
-- 10) WHERE clause using the LIKE operator with the percent wildcard character preceding the literal characters. 
...
-- 11) WHERE clause using the LIKE operator with the percent wildcard character preceding and following the literal characters. 
...
-- 12) WHERE clause using the LIKE operator with the underscore wildcard character preceding and following a literal character.
...
-- 13) WHERE clause using the LIKE operator with the underscore wildcard character preceding and the percent sign following a literal character.
...
-- 14) If you want to match a literal underscore or percent sign without matching other characters then 
...
-- 15) Following from the preceding example, if the same query is used except that the escape character is excluded then
...
-- 16) Select Statement 1: Without Parentheses
...
-- 17) Select Statement 2: With Parentheses
